...
var jogadorAtual = jogador1;
//Colocando o jogo para funcionar somente quando há clicks na área da grade
document.getElementById('container').addEventListener('click', function () {
    console.log(novaPos);
    console.log(jogadorAtual);

    if (jogoContinua === true) {
        //Ocupar a celula a primeira vez
        //var ocupouACelula = ocuparCelula(jogadorAtual, novaPos);
        //Verificar se celula está ocupada e tentar ocupar até consguir
        var ocupouACelula = false;

        while (jogoContinua === false){
          while(ocupouACelula === false){
              ocupouACelula = ocuparCelula(jogadorAtual, novaPos);
              jogoContinua = false;
          }
        ...
        var vencedor = verificarSeGanhou(jogadorAtual);
        if (vencedor === true) {
            jogoContinua = false;
