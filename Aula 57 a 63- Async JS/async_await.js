...
//Com async await
async function obterDadosAW(){
  //Consumindo a primeira promessa
  const identidade = await obterIDs;

  const dados = await obterDadosPessoa(identidade);

  const idade = await obterIdadePessoa(dados);

  return dados;

}
//obterDadosAW();
//const info = obterDadosAW();
//console.log(info);
let res;
obterDadosAW().then(resultado => {
